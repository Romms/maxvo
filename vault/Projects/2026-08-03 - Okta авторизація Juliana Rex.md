...
## Пряме рішення Романа (04.08) — новий поворот, третій варіант з'явився

Роман діяв, не чекаючи довше:
1. **Написав Okta-реселеру** з питанням про ціну й можливий trial Custom Auth Server (API Access
   Management) — **очікує відповіді**.
2. **Паралельно зробив MVP на [Zitadel](https://zitadel.com/)** (open-source/self-hosted IAM, третій
   варіант, якого не було в дослідженні вище) для **scribe-mcp та decision-mcp** — і воно вже
   запрацювало.
3. Наступний крок, який Роман намітив: переписати ці застосунки на Zitadel остаточно, але **не
   прив'язуватись жорстко до Zitadel-специфіки** — щоб можна було замінити на Okta пізніше, якщо
   реселер дасть прийнятну ціну/trial.

### Відповідь Романа (04.08): обидва одразу

Це **і** реальна заміна Auth0 для scribe-mcp/decision-mcp конкретно (не просто proof-of-concept), **і**
пілот — якщо підхід себе виправдає, той самий vendor-abstracted auth layer (Zitadel зараз, Okta
пізніше якщо реселер дасть прийнятні умови) поширюється й на Juliana/Rex.

Тобто три паралельні треки одного рядка в Активні:
1. **scribe-mcp/decision-mcp → Zitadel** — MVP вже працює, лишилось довести до продакшн-стану
   (прибрати жорстку прив'язку до Zitadel-специфіки в коді).
2. **Okta-реселер** — очікує відповіді про ціну/trial Custom Auth Server.
3. **Juliana/Rex** — рішення відкладено до результату (1) і (2): якщо Zitadel-підхід у
   scribe-mcp/decision-mcp пройде добре, і/або Okta-реселер не дасть прийнятних умов — Zitadel-шлях
   поширюється й сюди.

## Track 1 успішний, track 2 рухається — паралельний POC на Okta (07.08)

**Track 1**: POC на Zitadel для scribe-mcp/decision-mcp пройшов вдало — деталі та наступний крок
(портал для Railway-проєктів) тепер у [[2026-08-07 - Zitadel-Railway auth portal]].

**Track 2**: Okta дають trial на Custom Authentication Server (API Access Management) — той самий
add-on, що був головним блокером усього дослідження вище. Роман хоче зробити паралельний POC на
Okta (той самий обсяг, що робився для Zitadel), щоб порівняти напряму, а не лише за прайсом.

**Причина розглядати консолідацію на одному вендорі**: підтримувати два продукти (Zitadel +
Okta Custom Auth Server) — додаткове адміністративне навантаження. Roman: Okta зараз — тільки
identity provider (SSO/логін), а авторизація — окрема система; тобто два різні бренди самі по собі
не страшні (розділення відповідальності вже є). Але Okta може бути простіше адмінити й оплачувати,
бо оплата вже є (не нова стаття витрат, на відміну від чистого Zitadel self-hosted, де довелось би
рахувати інфраструктурні витрати на хостинг окремо).

**Це прямо продовжує "Перегляд висновку (03.08)" і "Фінальний аналіз" вище** — там порівнювались
Auth0 vs Okta API Access Management за ціною ($2/user/міс) і складністю міграції; тепер до
порівняння додається третій, вже перевірений на практиці варіант (Zitadel), і сама Okta-опція
нарешті стає технічно доступною для реального POC замість лише прайс-листа.

**Наступний крок**: коли Okta trial активується — повторити той самий POC-обсяг, що на Zitadel, і
порівняти результати (складність налаштування, вартість підтримки, чи Custom Auth Server дійсно
знімає блокер з `E0000015`). Рішення "Zitadel vs Okta" ще не прийнято.
