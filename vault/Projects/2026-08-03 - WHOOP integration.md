...
## Діагностика: конектор недоступний у сесії (07.08 вночі)

Протягом 07.08 WHOOP-тули (`get_today` та решта) то з'являлись, то зникали з поточної Claude Code
сесії — непередбачувано. Роман перевірив claude.ai → Settings → Connectors: **"Whoop MCP" підключений
на рівні акаунта**, всі 6 tools перелічені (`get_auth_url`, `get_recovery_trends`, `get_sleep_analysis`,
`get_strain_history`, `get_today`, `sync_data`), дозвіл — "Needs approval". Тобто сам сервіс на Railway
живий, конектор зареєстрований — проблема не там.

Спроба disconnect/reconnect **в межах тієї самої сесії** не допомогла — тули так і не з'явились.
**Гіпотеза**: прив'язка конектора до сесії відбувається лише при **старті нової сесії**, не
підхоплюється заднім числом у вже запущеній. Не підтверджено остаточно — Роман планує перевірити,
відкривши нову сесію замість цієї. Якщо в новій сесії тули з'являться одразу — гіпотеза підтверджена,
і практичний висновок: якщо WHOOP-тули колись зникнуть знову, спершу пробувати нову сесію, а не
шукати проблему на боці Railway/сервісу.
