...
criterion, plus asking when to remind — `Check-in` + a `Нагадування` calendar event) or Ідеї (plus a
`YYYY-MM-DD - Title.md` detail file) if explicitly not a commitment; anything ambiguous goes to
`vault/Inbox.md` Unprocessed for the weekly triage.
