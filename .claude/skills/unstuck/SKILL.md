...
apply the technique and stop.

Grounded in (context, don't recite this to Roman each time):
- **Worry Tree (Butler & Hope, CBT)**: the key branch is whether the worry is something you can
  actually act on now, or hypothetical. Hypothetical → let it go. Actionable → make a concrete plan,
  then let it go.
- **Zeigarnik effect / "Consider it done" (Masicampo & Baumeister)**: the brain quiets intrusive
  thoughts about an unfinished thing not when it's done, but when there's a *specific plan* for it
  (who/what/when/where/how) — the plan itself, not completion, releases the tension.
...
- Записати в `vault/Open Loops.md` (якщо це задача) або призначити конкретний час рішення/розмови.
